...
public class MoveScriptRegression : MonoBehaviour {
	...
	void Start () {
		...
		for(int i =0; i<trainningExample.Length;i++) {
			...
			points[i*3+2] = trainningExample[i].position.y;
		} 
		LibWrapper.linear_train_regression(coeff,points,trainningExample.Length *3);
		
		foreach (var sphere in sphereTransforms){
			double[] point = {sphere.position.x,sphere.position.z};
			float result = (float)LibWrapper.regress_point(coeff,point);
